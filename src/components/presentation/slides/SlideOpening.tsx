    ...
    <SlideContainer isActive={isActive}>
      <div className="text-center space-y-8">
        ...
        <div className={`${isActive ? 'animate-fade-up delay-200' : 'opacity-0'}`}>
          <h1 className="text-7xl md:text-9xl font-black text-gradient tracking-tight">
            <AnimatedNumber value={11} isActive={isActive} /> milhões
          </h1>
        </div>

        {/* Subtitle */}
        <p className={`text-xl md:text-3xl text-foreground/80 max-w-3xl mx-auto leading-relaxed ${isActive ? 'animate-fade-up delay-400' : 'opacity-0'}`}>
        ...
        {/* Decorative elements */}
        <div className={`absolute bottom-10 left-1/2 -translate-x-1/2 ${isActive ? 'animate-fade-in delay-700' : 'opacity-0'}`}>
          <div className="flex items-center gap-2 text-muted-foreground text-sm">
            <span className="w-12 h-px bg-muted-foreground/30"></span>
