          ...
          <div className="space-y-6">
            {milestones.map((milestone, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`relative flex items-start gap-6 pl-16 md:pl-20 ${isActive ? `animate-slide-left ${milestone.delay}` : 'opacity-0'}`}
                  style={{ animationDelay: `${100 + index * 100}ms` }}
                >
                  {/* Icon circle */}
                  <div
                    className={`absolute left-0 w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center ${statusColors[milestone.status as keyof typeof statusColors]}`}
                  >
                  ...
                  {/* Content */}
                  <div
                    className={`flex-1 bg-card border rounded-xl p-5 md:p-6 ${
                      milestone.highlight 
