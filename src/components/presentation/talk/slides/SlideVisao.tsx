...
const academic = [
  { area: "HCI / Interação Humano-Computador", contribution: "Como populações vulneráveis aceitam e se apropriam de IA conversacional", color: "#E8673C" },
  { area: "Sistemas Inteligentes", contribution: "Arquitetura de SMA-c com LLMs para assistência financeira empática", color: "#C040A0" },
    ...
    <TalkSlideContainer className="bg-[#0A0A0A] flex relative overflow-hidden">
      ...
      <div className={`${isBaia ? "w-[55%]" : "w-full"} h-full flex flex-col pt-16 pb-16 pl-20 pr-16 z-10 justify-between`}>
        <div>
          ...
          <h2 className="font-display text-[72px] font-black text-white leading-tight">
            Para onde a{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Yá vai</span>
          </h2>
        </div>
      ...
      {isBaia && (
        <div className="w-[45%] h-full flex flex-col justify-center pr-20 z-10">
          <h3 className="font-display text-[28px] font-bold text-white mb-8">Contribuição Acadêmica</h3>
          <div className="flex flex-col gap-5">
            {academic.map((a, i) => (
